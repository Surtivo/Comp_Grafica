...
void Teclado(unsigned char key, int x, int y)
{
    ...
    switch (key)
    {
    case '1': varObj = 1; break; // Seleciona Retangulo
    case '2': varObj = 2; break; // Seleciona Losango
    case '3': varObj = 3; break; // Seleciona Elipse
    case 'R':
    case 'r': corAtiva = 1; break; // Seleciona cor Vermelho
    case 'G':
    case 'g': corAtiva = 2; break; // Seleciona cor Azul
    case 'B':
    case 'b': corAtiva = 3; break; // Seleciona cor Amarelo
    }
    glutPostRedisplay();
}
...
void TeclasEspeciais(int key, int x, int y)
{
    ...
    if (varObj == 1)      { px = &posX_Retangulo; py = &posY_Retangulo; }
    else if (varObj == 2) { px = &posX_Losango;   py = &posY_Losango; }
    else if (varObj == 3) { px = &posX_Elipse;    py = &posY_Elipse; }

    if (px != NULL && py != NULL)
    {
        switch (key)
        {
        case GLUT_KEY_LEFT:  *px -= passo; break;
        case GLUT_KEY_RIGHT: *px += passo; break;
        case GLUT_KEY_UP:    *py += passo; break;
        case GLUT_KEY_DOWN:  *py -= passo; break;
        }
    }
